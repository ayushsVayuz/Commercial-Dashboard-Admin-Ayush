...
import { fetchActiveFormListing } from "../../../redux/actions/form-action";
...
function HelperMfListing() {
  ...
  useEffect(() => {
    if (searchValue?.length > 0) {
      setCurrentPage(0);
      dispatch(fetchHelper([{ field: "query", value: searchValue }]));
    } else {
      dispatch(fetchHelper([{ field: "page", value: currentPage + 1 }]));
    }
  }, [currentPage, searchValue]);

  // data and api calls

  const { helperData, isToggleLoading, loading } = useSelector(
    (state) => state.helper
  );
  console.log(helperData, loading);

  const dataToPass = helperData?.map((data, i) => ({
    id: data.helperMicroFrontendId,
    sno: i + 1,
    containerId: titleCase(data.containerId),
  ...
  const getActionMenu = (id) => {
    ...
    return [
      {
        label: "Edit",
        url: `/helper/edit/${encrypt(id)}`,
        icon: <TbEdit className="text-xl" />,
      },
      {
        label: "View",
        url: `/helper-detail/${encrypt(id)}`,
  ...
  return (
    <>
      <section className="flex flex-col gap-4">
        <MetaTitle title={"Helper MF | Anarock"} />
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          <Heading
            sectionLink="/helper"
            parent="Helper Form Listing"
            mainTitle="Helper Form Listing"
          />
          <Link
...
export default HelperMfListing;
