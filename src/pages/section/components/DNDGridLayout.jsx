...
export default function DNDGridLayout({
  ...
  function makeLayout(items) {
    return items?.map((w) => {
      const mapping = AllWidgetMapping[w?.container_id] || {};
      const [defX, defY, defW, defH] = mapping.position || [0, 0, 1.5, 1.5];

      // Use item's position if available, otherwise use defaults
      const x = w?.position?.[0] ?? defX;
      const y = w?.position?.[1] ?? defY;
      const w_val = Math.max(w?.position?.[2] ?? defW, mapping.minWidth) || 1.5;
      const h_val = Math.max(w?.position?.[3] ?? defH, mapping.minHeight) || 1.5;
  ...
  const handleLayoutChange = (currentLayout) => {
    const updated = currentLayout.map((l) => {
      const matchedWidget = effectiveData?.find((item) => item?.widget_id === l?.i);
      return {
        widget_id: l.i,
        widget_name: matchedWidget?.widget_name || "",
        container_id: matchedWidget?.container_id || "",
