...
export const encrypt = (text) => {
  ...
  const urlSafeEncryptedText = encryptedText
    .replace(/\//g, "_")
    .replace(/\+/g, "-")
    .replace(/=+$/, ""); // Trim padding characters
...
export const decrypt = (cipherText) => {
  if (cipherText) {
    const originalCipherText = cipherText.replace(/_/g, "/").replace(/-/g, "+");

    // Add padding if necessary
    const padLength = 4 - (originalCipherText.length % 4);
    const paddedCipherText =
      originalCipherText + "=".repeat(padLength === 4 ? 0 : padLength);

    try {
      const bytes = CryptoJS.AES.decrypt(paddedCipherText, secretKey);
...
export function removeAfterFourthWhitespace(inputString) {
  ...
  // Take the first four parts and join them back with whitespace
  let result = parts.slice(0, 5).join(" ") + "...";
...
export function removeAfterTenthWhitespace(inputString) {
  ...
  // Take the first four parts and join them back with whitespace
  let result = parts.slice(0, 11).join(" ") + "..";
...
export function removeAfterTwentiethWhitespace(inputString) {
  ...
  // Take the first twenty-one parts and join them back with whitespace
  let result = parts.slice(0, 21).join(" ") + "...";

  return result;
}
export function removeAfterFourtithWhitespace(inputString) {
  // Split the string based on whitespace
  let parts = inputString.split(/\s+/);

  // Take the first twenty-one parts and join them back with whitespace
  let result = parts.slice(0, 41).join(" ") + "...";
